...
- **modelAliases:** Map of request model aliases to real Copilot model IDs before `/v1/responses` endpoint validation. This is mainly for Codex auto-permission mode, whose reviewer turns can use internal model names such as `codex-auto-review`, `auto_review`, or `guardian_subagent`. The built-in aliases route legacy reviewer names to `gpt-5-mini` and `codex-auto-review` to `gpt-5.2`, because Codex can send approval prompts larger than `gpt-5-mini`'s 128k prompt-token limit.
